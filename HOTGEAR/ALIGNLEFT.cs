...
namespace DnBim_Tool
{
    ...
    public class AlignTagLeftCmd : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                {
                    Reference r = uidoc.Selection.PickObject(ObjectType.Element, "Chọn tag hoặc text để align left");
                    if (r == null) break;
                ...
                double GetLeftX(Element e)
                {
                    BoundingBoxXYZ box = e.get_BoundingBox(uidoc.Document.ActiveView);
                    return box?.Min.X ?? double.MaxValue;
                }

                double leftX = selectedTags.Min(GetLeftX);

                using (Transaction trans = new Transaction(doc, "Align Tags to Left"))
                {
                    trans.Start();

                    foreach (var tag in selectedTags)
                    {
                        double currentX = GetLeftX(tag);
                        double deltaX = leftX - currentX;
